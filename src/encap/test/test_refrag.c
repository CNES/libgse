...
static int test_encap(int verbose,
                              char *src_filename,
                              char *cmp_filename);
                      ...
                      char *cmp_filename)
{
  ...
  while((packet = (unsigned char *) pcap_next(handle, &header)) != NULL)
  {
    ...
    status = gse_refrag_packet(vfrag, &vfrag_pkt, qos, REFRAG_LENGTH);
    if((status != STATUS_OK))
    {
      DEBUG(verbose, "Error %d when refragment packet\n", status);
      goto release_lib;
    }
